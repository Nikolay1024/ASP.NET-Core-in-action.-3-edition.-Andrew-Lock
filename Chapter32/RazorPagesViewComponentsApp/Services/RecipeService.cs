...
namespace RazorPagesViewComponentsApp.Services
{
    public class RecipeService
    {
        ...
        public async Task<List<RecipeSummaryViewModel>> GetRecipesByUser(string createdBy, int numberOfRecipes)
        {
            return await _context.Recipes.Where(r => r.CreatedBy == createdBy && !r.IsDeleted)
                .OrderBy(r => r.LastModified).Take(numberOfRecipes)
                .Select(r => new RecipeSummaryViewModel()
                {
                    Id = r.RecipeId,
                    Name = r.Name,
        ...
        public async Task<int> CreateRecipe(RecipeCreateCommand cmd, AppUser createdBy)
        {
        ...
        public async Task<RecipeUpdateCommand?> GetRecipeForUpdate(int id)
        {
            return await _context.Recipes.Where(r => r.RecipeId == id && !r.IsDeleted)
                .Select(r => new RecipeUpdateCommand()
                {
                    Name = r.Name,
                    Method = r.Method,
        ...
        public async Task<bool> DoesRecipeExist(int id) =>
            await _context.Recipes.Where(r => !r.IsDeleted && r.RecipeId == id).AnyAsync();
