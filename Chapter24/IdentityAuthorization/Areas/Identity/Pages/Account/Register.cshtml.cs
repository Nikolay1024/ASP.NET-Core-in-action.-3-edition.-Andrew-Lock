...
namespace IdentityAuthorizationApp.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private async Task AddClaims(IdentityUser user)
        {
            ...
            {
                var claim = new Claim(ClaimTypes.DateOfBirth, Input.DateOfBirth.Value.ToString("yyyy-MM-dd"), ClaimValueTypes.Date);
                await _userManager.AddClaimAsync(user, claim);
            }
            if (!string.IsNullOrEmpty(Input.EmployeeNumber))
            {
                var claim = new Claim(Claims.EmployeeNumber, Input.EmployeeNumber);
                await _userManager.AddClaimAsync(user, claim);
            }
            if (!string.IsNullOrEmpty(Input.BoardingPassNumber))
            {
                var claim = new Claim(Claims.BoardingPassNumber, Input.BoardingPassNumber);
                await _userManager.AddClaimAsync(user, claim);
            }
            if (!string.IsNullOrEmpty(Input.FrequentFlyerClass))
            {
                var claim = new Claim(Claims.FrequentFlyerClass, Input.FrequentFlyerClass);
                await _userManager.AddClaimAsync(user, claim);
            }
            if (Input.IsBannedFromLounge)
            {
                var claim = new Claim(Claims.IsBannedFromLounge, "true");
                await _userManager.AddClaimAsync(user, claim);
            }
        }
    }
